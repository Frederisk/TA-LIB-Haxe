...
@:keep
function CdlHammer(startIndex:Int, endIndex:Int, inOpen:Array<Float>, inHigh:Array<Float>, inLow:Array<Float>, inClose:Array<Float>) {
    var outBegIndex:Int;
    ...
    do {
        if (RealBody(i, inOpen, inClose) < CandleAverage(CandleSettingType.BodyShort, BodyPeriodTotal, i, inOpen, inHigh, inLow, inClose)
            ...
            && Math.min(inClose[i],
                inOpen[i]) <= inLow[i - 1] + CandleAverage(CandleSettingType.Near, NearPeriodTotal, i - 1, inOpen, inHigh, inLow, inClose)) {

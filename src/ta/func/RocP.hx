...
@:keep
function RocP(startIndex:Int, endIndex:Int, inReal:Array<Float>, optInTimePeriod:Int) {
    var outBegIndex:Int;
    ...
    while (inIndex <= endIndex) {
        ...
        if (tempReal != 0.0) {
            outReal[outIndex++] = (inReal[inIndex] - tempReal) / tempReal;
        } else {
...
@:keep
function RocPLookback(optInTimePeriod:Int):Int {
    // INTEGER_DEFAULT
    // if(optInTimePeriod == null || ){
    //     optInTimePeriod = 10;
    // } else

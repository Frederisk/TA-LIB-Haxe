...
@:keep
function CdlKickingByLength(startIndex:Int, endIndex:Int, inOpen:Array<Float>, inHigh:Array<Float>, inLow:Array<Float>, inClose:Array<Float>) {
    var outBegIndex:Int;
    ...
    do {
        if (CandleColor(i - 1, inOpen, inClose) == -CandleColor(i, inOpen, inClose)
            ...
            outInteger[outIndex++] = CandleColor((RealBody(i, inOpen, inClose) > RealBody(i - 1, inOpen, inClose) ? i : i - 1), inOpen, inClose) * 100;
        } else {
...
@:keep
function CdlKickingByLengthLookback():Int {

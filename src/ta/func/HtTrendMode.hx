...
@:keep
function HtTrendMode(startIndex:Int, endIndex:Int, inReal:Array<Float>) {
    var outBegIndex:Int;
    ...
    var DCPhase:Float, DCPeriod:Float, imagPart:Float, realPart:Float;

    var daysInTrend:Int, trend:Int;
    var prevDCPhase:Float, trendline:Float;
    var prevSine:Float, prevLeadSine:Float, sine:Float, leadSine:Float;
    ...
    lookbackTotal = 63 + Globals.unstablePeriod[FuncUnstId.HtTrendMode];
    ...
    while (today <= endIndex) {
        ...
        index = smoothPrice_Index;

        i = 0;
        while (i < DCPeriodInt) {
            tempReal = (i * constDeg2RadBy360) / DCPeriodInt;
            tempReal2 = smoothPrice[index];
            realPart += Math.sin(tempReal) * tempReal2;
            imagPart += Math.sin(tempReal) * tempReal2;
            if (index == 0) {
                index = SMOOTH_PRICE_SIZE - 1;
            } else {
                index--;
            }
            i++;
        ...
        tempReal = smoothPrice[smoothPrice_Index];
        if ((trendline != 0.0) && (Math.abs((tempReal - trendline) / trendline) >= 0.015)) {
            trend = 1;
        }

        if (today >= startIndex) {
            outInteger[outIndex++] = trend;
        }

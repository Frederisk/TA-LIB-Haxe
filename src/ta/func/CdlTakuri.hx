...
@:keep
function CdlTakuri(startIndex:Int, endIndex:Int, inOpen:Array<Float>, inHigh:Array<Float>, inLow:Array<Float>, inClose:Array<Float>) {
    var outBegIndex:Int;
    ...
    var BodyDojiPeriodTotal:Float,
        ...
        ShadowVeryLongPeriodTotal:Float;
    var i:Int,
        outIndex:Int,
        BodyDojiTrailingIndex:Int,
        ShadowVeryShortTrailingIndex:Int,
        ShadowVeryLongTrailingIndex:Int,
        lookbackTotal:Int;
    ...
    ShadowVeryLongTrailingIndex = startIndex - Globals.candleSettings[CandleSettingType.ShadowVeryLong].avgPeriod;
    ...
    i = ShadowVeryLongTrailingIndex;
    while (i < startIndex) {
        ShadowVeryLongPeriodTotal += CandleRange(CandleSettingType.ShadowVeryLong, i, inOpen, inHigh, inLow, inClose);
        i++;
    }
    ...
    do {
        if (RealBody(i, inOpen, inClose) <= CandleAverage(CandleSettingType.BodyDoji, BodyDojiPeriodTotal, i, inOpen, inHigh, inLow, inClose)
            ...
            && LowerShadow(i, inOpen, inLow,
                inClose) > CandleAverage(CandleSettingType.ShadowVeryLong, ShadowVeryLongPeriodTotal, i, inOpen, inHigh, inLow, inClose)) {
        ...
        ShadowVeryLongPeriodTotal += CandleRange(CandleSettingType.ShadowVeryLong, i, inOpen, inHigh, inLow, inClose)
            - CandleRange(CandleSettingType.ShadowVeryLong, ShadowVeryLongTrailingIndex, inOpen, inHigh, inLow, inClose);
        i++;
        BodyDojiTrailingIndex++;
        ShadowVeryShortTrailingIndex++;
        ShadowVeryLongTrailingIndex++;
    } while (i <= endIndex);
...
@:keep
function CdlTakuriLookback():Int {
    return TAIntMax(TAIntMax(Globals.candleSettings[CandleSettingType.BodyDoji].avgPeriod, Globals.candleSettings[CandleSettingType.ShadowVeryShort].avgPeriod),
        Globals.candleSettings[CandleSettingType.ShadowVeryLong].avgPeriod);
}

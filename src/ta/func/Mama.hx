...
@:keep
function Mama(startIndex:Int, endIndex:Int, inReal:Array<Float>, optInFastLimit:Float, optInSlowLimit:Float) {
    var outBegIndex:Int;
    var outNBElement:Int;
    var outMAMA:Array<Float> = [];
    var outFAMA:Array<Float> = [];
    ...
    var mama:Float, fama:Float, todayValue:Float, prevPhase:Float;
    ...
    //    optInFastLimit = 0.5;
    // }
    if (optInFastLimit <= 0 || optInFastLimit >= 1) {
        throw new TAException(BadParam);
    }
    // INTEGER_DEFAULT
    // if(optInSlowLimit = null) {
    //    optInFastLimit = 0.5;
    // }
    if (optInSlowLimit <= 0 || optInSlowLimit >= 1) {
        throw new TAException(BadParam);
    }

    rad2Deg = 180.0 / (4.0 * Math.atan(1));

    lookbackTotal = 32 + Globals.unstablePeriod[FuncUnstId.Mama];
    ...
    while (today <= endIndex) {
        ...
        if (today >= startIndex) {
            outMAMA[outIndex] = mama;
            outFAMA[outIndex++] = fama;
        }
...
function MamaLookback(optInFastLimit:Float, optInSlowLimit:Float) {
    ...
    return (32 + Globals.unstablePeriod[FuncUnstId.Mama]);
}

...
@:keep
function HtDcPhase(startIndex:Int, endIndex:Int, inReal:Array<Float>) {
    var outBegIndex:Int;
    ...
    var rad2Deg:Float, constDeg2RadBy360:Float;
    ...
    lookbackTotal = 63 + Globals.unstablePeriod[FuncUnstId.HtDcPhase];
    ...
    while (today <= endIndex) {
        ...
        if (today >= startIndex) {
            outReal[outIndex++] = DCPhase;
        }

        // CIRCBUF_NEXT(Id) :
        // #1#_Index++;
        // if(#1#_Index > maxIndex_#1#) {
        //     #1#_Index = 0;
        // }

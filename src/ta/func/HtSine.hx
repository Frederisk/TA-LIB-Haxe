...
@:keep
function HtSine(startIndex:Int, endIndex:Int, inReal:Array<Float>) {
    var outBegIndex:Int;
    var outNBElement:Int;
    var outSine:Array<Float> = [];
    var outLeadSine:Array<Float> = [];
    ...
    lookbackTotal = 63 + Globals.unstablePeriod[FuncUnstId.HtSine];
    ...
    while (today <= endIndex) {
        ...
        if (today >= startIndex) {
            outSine[outIndex] = Math.sin(DCPhase * deg2Rad);
            outLeadSine[outIndex++] = Math.sin((DCPhase + 45) * deg2Rad);
        }
